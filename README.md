...
| Endpoint | Description |
| --- | --- |
| `POST /cooperatives/:id/ai/assistant` | Ask a question; answered from the requester's own membership/savings/loans/upcoming-meetings data (any active member), plus the cooperative-wide dashboard KPIs if the requester's role has dashboard access (`VIEW_DASHBOARD_ROLES`) |
